...
class ExponentialSmoothingModel(BaseModel):
    ...
    def fit(self, data: pd.Series) -> None:
        """Fit Exponential Smoothing model with smart seasonal period selection"""
        ...
        try:
            ...
            # Smart seasonal period selection based on data length
            optimal_period = self._select_optimal_seasonal_period(data)
            self.best_seasonal_period = optimal_period

            # Update configuration with optimal seasonal period
            config = self.best_config.copy()
            config['seasonal_periods'] = optimal_period

            logger.info(f"Selected seasonal period: {optimal_period} (data length: {len(data)})")

            # Fit model with optimized configuration
            fitted_model = self._fit_single_config(data, config)

            if fitted_model is None:
                raise ValueError("Could not fit Exponential Smoothing with optimized configuration")
    ...
    def _intelligent_seasonal_selection(self, data: pd.Series) -> List[int]:
        ...
        return [self._select_optimal_seasonal_period(data)]
